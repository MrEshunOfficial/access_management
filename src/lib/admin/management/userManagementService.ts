...
export async function suspendUser(
  ...
  try {
    ...
    if (
      ...
      return {
        success: false,
        error: "Cannot suspend admin users unless you are super admin",
      };
    }
    ...
    if (updatedUser) {
      ...
      await AdminAuditLog.create({
        ...
        details: `User suspended. Reason: ${data.reason}${
          data.duration ? `, Duration: ${data.duration} days` : " (Permanent)"
        }`,
      });

      // Note: With JWT-based sessions, we cannot force logout users from server-side.
      // The user's JWT tokens will remain valid until they naturally expire.
      // Consider implementing middleware to check user status on each request if immediate logout is required.
...
export async function blockUser(
  ...
  try {
    ...
    if (
      ...
      return {
        success: false,
        error: "Cannot block admin users unless you are super admin",
      };
    }
...
export async function getAllUsers(filters: UserListFilters = {}) {
  try {
    ...
    const { role, status = "active", page = 1, limit = 50, search } = filters;
    ...
    const [users, total] = await Promise.all([
      User.find(query)
        .select(
          "email name role status createdAt updatedAt suspendedBy suspendedAt blockReason"
        )
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(limit)
...
export async function restoreUser(
  ...
  try {
    ...
    // Restore the user by setting status back to active
    await User.findByIdAndUpdate(userId, {
      status: "active",
      deletedAt: null, // Clear deletion timestamp if you have one
      // Optionally clear any deletion-related fields
      deletionReason: null,
      deletedBy: null,
    });
    ...
    await AdminAuditLog.create({
      ...
      details: `User restored. Reason: ${reason}`,
      ipAddress,
      timestamp: new Date(),
    });

    // Optionally send notification email to the restored user
    // await sendUserRestorationEmail(user.email);
